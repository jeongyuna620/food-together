...
export default function RoomPage() {
  ...
  const [customCantEat, setCustomCantEat] = useState('')
  ...
  const checkAndRedirect = useCallback(async () => {
    ...
    if (!room) { setRoomStatus('notfound'); return }
    if (room.status === 'results') { router.replace(`/room/${code}/results`); return }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const allCantEat = [...cantEat]
      if (customCantEat.trim()) allCantEat.push(customCantEat.trim())
      ...
      <div className="max-w-md mx-auto px-4 pt-5 space-y-4">
        ...
        <section className="bg-white rounded-2xl p-4 shadow-sm">
          ...
          <input
            type="text"
            value={customCantEat}
            onChange={e => setCustomCantEat(e.target.value)}
            placeholder="직접 입력 (예: 땅콩, 새우)"
            className="w-full border-2 border-gray-100 rounded-xl p-3 mt-2 text-sm focus:outline-none focus:border-orange-400"
          />
        </section>
        ...
        <section className="bg-white rounded-2xl p-4 shadow-sm">
          ...
          <button
            ...
            className={`w-full py-3 rounded-xl border-2 font-medium text-sm transition-colors ${
              locationStatus === 'done' ? 'border-green-400 bg-green-50 text-green-700' :
              locationStatus === 'error' ? 'border-gray-200 bg-gray-50 text-gray-400' :
              'border-blue-200 bg-blue-50 text-blue-600'
            }`}
          >
            {locationStatus === 'idle'    && '📍  위치 감지하기'}
            {locationStatus === 'loading' && '📍  감지 중...'}
            {locationStatus === 'done'    && '✅  위치 감지 완료'}
            {locationStatus === 'error'   && '⚠️  감지 실패 — 계속 진행 가능'}
          </button>
        </section>
      </div>
